...
class Shrek extends Character {
	...
	figth<Type extends {name: string}>(enemy: Type): void {
		console.log(`${this.name} is fighting ${enemy.name}`);
	}

	eat<T extends string[]> (listOfFood: T): void {
		console.log(`${this.name} is eating: ${[...listOfFood]}`);
	}
}
